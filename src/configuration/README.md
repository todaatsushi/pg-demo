...
```bash
psql -U postgres -f /app/src/configuration/0001_databases/run.sql
```

4. Apply steps 2–4 in order against `grocery_store`:

```bash
psql -U postgres -d grocery_store -f /app/src/configuration/0002_data/run.sql
psql -U postgres -d grocery_store -f /app/src/configuration/0003_organisation/run.sql
psql -U postgres -d grocery_store -f /app/src/configuration/0004_users/run.sql
```

5. Run the step 5 Python setup from `src/configuration/0005_app/src` (uses the correct role per operation):
...
psql -U postgres -d grocery_store -f /app/src/configuration/0004_users/reverse.sql
psql -U postgres -d grocery_store -f /app/src/configuration/0003_organisation/reverse.sql
psql -U postgres -d grocery_store -f /app/src/configuration/0002_data/reverse.sql
psql -U postgres -f /app/src/configuration/0001_databases/reverse.sql
```
